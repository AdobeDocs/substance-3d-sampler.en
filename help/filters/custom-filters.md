...
Filters must be built in a specific way in Designer to work correctly once imported into Sampler.

The input and output nodes of the filter must have an identifier or usage defined.
...
>[!IMPORTANT]
>
> If, in your package, you have one graph to process images (scan1 to scanX) and one graph to process materials (PBR channels), Sampler is able to choose the correct graph depending on where the filter is inserted in the layer stack.
>
> In your "image" graph, add the following userdata:
>
...
Specific parameters are globally managed by the application. It's a way to use global parameters of the application, the project, and the layer stack in your custom filters.
...
If you want to display/hide some parameters based on the workflow of your project (PBR Metalic/Roughness or PBR Specular/Glossiness), you can use the Workflow Type parameter

**Parameter identifier**: workflow_type

**Parameter type**: integer1, dropdown list
